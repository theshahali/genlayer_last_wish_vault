...
class LastWishVault(gl.Contract):
    ...
    def create_vault(
        ...
        new_vault = VaultRecord(
            ...
            failed_audits_count=u256(0),
            quality_score=u256(100),
            status="ACTIVE_LIVE",
            last_audit_summary=f"Liveness decision gate initialized. Beneficiary: {ben_clean}. Grace period: {grace_period_days} days."
        )
    ...
    def audit_liveness_heartbeat(self, vault_id: str) -> None:
        ...
        assert vault.status in ("ACTIVE_LIVE", "GRACE_PERIOD_WARNING"), "Vault decision audit is already finalized."
        ...
        task = (
            "You are a Senior Decentralized Liveness & Succession Auditor for dead man's switch decision gates.\n"
        ...
        # No assert revert on unreachable — non-reachability is a valid liveness state!
        failed_count = int(vault.failed_audits_count)

        if liveness and score_val >= 70:
            # Heartbeat verified alive -> Reset status to ACTIVE_LIVE and reset failed count
            vault.status = "ACTIVE_LIVE"
            vault.failed_audits_count = u256(0)
            ...
            # Heartbeat failed -> Advance Grace Period / Succession Ready state machine
            failed_count += 1
            vault.failed_audits_count = u256(failed_count)
            vault.quality_score = u256(score_val)

            if failed_count == 1:
                # 1st Failure -> Transition to GRACE_PERIOD_WARNING
                vault.status = "GRACE_PERIOD_WARNING"
                vault.last_audit_summary = f"GRACE PERIOD WARNING (Failure 1): Heartbeat unverified. Vault entered grace period window. " + summary
            else:
                # Subsequent Failure -> Finalize to SUCCESSION_READY_INACTIVE
                vault.status = "SUCCESSION_READY_INACTIVE"
                vault.last_audit_summary = f"SUCCESSION READY INACTIVE (Failure {failed_count}): Inactivity verified. Succession decision unlocked. " + summary
    ...
    def is_succession_ready(self, vault_id: str) -> bool:
        assert vault_id in self.vaults, "Vault ID does not exist."
        return self.vaults[vault_id].status == "SUCCESSION_READY_INACTIVE"
